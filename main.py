...
def start_quiz():
  ...
  sessions[session_id] = QuizSession(questions=questions)
  return {
...
def submit_answer(session_id: str, payload: AnswerInput):
  ...
  question = next((q for q in session.questions if q.id == payload.questionId),
                  None)
  if question is None:
...
def get_result(session_id: str):
  ...
  score = sum(
      ...
      if session.answers.get(q.id) == q.correctAnswerId
  )
  return {
    "score": score,
